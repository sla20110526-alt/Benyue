...
SOURCE = ROOT / "文本" / "奔月_正式分镜稿_V1.1.docx"
OUTPUT = ROOT / "文本" / "奔月_正式分镜稿_V1.2.docx"
QA_OUTPUT = Path(__file__).with_name("v12_qa_report.json")
...
def add_cover(doc, total_shots):
    ...
    add_paragraph(doc, f"{VERSION}  |  正片 14分00秒  |  16场  |  {total_shots}镜", size=10.5, bold=True,
    ...
    add_kv_table(doc, [
        ("正式基线", "文本/奔月_正式分镜稿_V1.2.docx；V1.1转为历史基线。"),
        ("任务日期", LANDING_DATE),
        ("着陆地点", LANDING_SITE),
        ("画面规格", "16:9；成片3840×2160；24fps；未来纪实、真实工程质感、AI真人。"),
        ("声音锁定", "全片无配乐；对白、无线电、设备声与任务通信随镜头同步生成；月面外部无空气传播声。"),
        ("叙事起点", "从揽月接近并对接梦舟开始，不再回溯发射；完整覆盖转移、登月、采样、离月、再对接、返地、再入和搜救。"),
        ("高优先级待决", "2029年4月8日与锁定着陆点自然光照条件存在天文冲突。V1.2保留日期，但不锁定太阳方位；进入画面生产前必须确认处理口径。"),
    ])
...
def add_control_section(doc):
    ...
    add_kv_table(doc, [
        ...
        ("时间压缩", "数小时月面驻留、数小时交会和三天地月巡航以时间字幕、延时、轨迹图与匹配剪辑压缩；关键节点保持实时质感。"),
        ...
        ("新闻解释", "记者只承担必要背景和一条国旗材料科普，不介入任务口令链。"),
    ])
...
def add_roles_section(doc):
    ...
    add_paragraph(doc, "2.3 地面系统与艺术化呼号", size=9, bold=True, color=BLUE, keep=True, before=3)
    table = doc.add_table(rows=1, cols=3)
...
def add_overview(doc):
    ...
    add_paragraph(doc, f"总计：16场，114镜，{TARGET_SECONDS}秒（14分00秒）。片尾推演声明不计入正片。",
...
def add_professional_check(doc):
    ...
    add_paragraph(
        doc,
        "V1.2已把任务构型、人员岗位、地面系统、月面活动、起飞再对接和返地搜救统一为一条闭合链路，具备进入资产重分析的脚本条件。进入正式画面生产前仍须解决一项高优先级问题：2029年4月8日与锁定着陆点自然光照不匹配。若日期不变，应明确这是艺术化光照；若坚持真实硬日光，应调整任务日期。",
        size=8.5,
        color="26384A",
        after=4,
    )
...
def add_sources(doc):
    ...
    add_paragraph(doc, "V1.2修订要点", size=9.5, bold=True, color=BLUE, keep=True, before=5)
    revision_points = [
        "1. 影片起点改为揽月接近并对接梦舟，完整补齐人员转移与双器分工。",
        "2. 删除服务舱载人观察室设定；刘定槎在梦舟返回舱紧凑工作位执行环月留守任务。",
        "3. 两张空座椅只等待登月乘组归队，不再承担主要视觉母题。",
        "4. 补齐揽月推进舱、月面起飞、再对接、乘组/样品转移和揽月受控处置。",
        "5. 补齐梦舟服务舱地月转移、近地分离、半弹道跳跃式再入、开伞/气囊和搜救闭环。",
        "6. 北京、林海、叶河、天衡和搜救系统进入关键节点，小天仅提供受限辅助。",
        "7. 国旗科普压缩为记者一句话；删除原稿中延伸到未来月球建设的材料预演表述。",
        "8. 删除全部配乐；逐镜落实月面/太空外部无空气传播声。",
    ]
...
def build():
    ...
    doc.core_properties.title = "《奔月》正式分镜稿 V1.2"
    doc.core_properties.subject = "中国首次载人登月未来纪实推演"
    doc.core_properties.author = "《奔月》项目组"
    doc.core_properties.comments = "以V1.1为版式模板重构；公开资料与艺术化推演边界见正文。"
